...
# 音频文件也支持：mp3 之类可以直接转字幕
AUDIO_EXT = {".mp3", ".m4a", ".wav", ".flac", ".aac", ".wma", ".opus", ".ogg", ".ape"}
...
def api_data(body):
    """解析接口返回；出错时给出可读的信息（而不是一个 KeyError: 'data'）。"""
    text = body.decode("utf-8", "replace") if isinstance(body, bytes) else str(body)
    try:
        obj = json.loads(text)
    except Exception:
        raise RuntimeError("接口返回不是 JSON：%s" % text[:300])
    if not isinstance(obj, dict):
        raise RuntimeError("接口返回格式异常：%s" % text[:300])
    data = obj.get("data")
    if data is None:
        raise RuntimeError("接口报错 code=%s message=%s"
                           % (obj.get("code"), obj.get("message") or obj.get("msg") or text[:200]))
    return data
...
def _transcribe_chunk_once(chunk_path, label=""):
    ...
    while time.time() < deadline:
        ...
        waited = time.time() - t0
        if waited >= next_note:
            log("    %s还在识别中（已等 %d 秒，B站那边处理中，属正常）" % (label, int(waited)))
            next_note += 30
        time.sleep(POLL_INTERVAL)
    raise RuntimeError("识别超时")


def transcribe_chunk(chunk_path, attempts=4, label=""):
    """识别单个音频分片；失败会自动重试，接口限流时很有用。"""
    last = None
    for i in range(attempts):
        try:
            return _transcribe_chunk_once(chunk_path, label)
        except Exception as exc:
            last = exc
            if i >= attempts - 1:
                break
            wait = min(300, 30 * (2 ** i))
            log("    %s识别失败：%s；%d 秒后重试（%d/%d）" % (label, exc, wait, i + 1, attempts))
            time.sleep(wait)
    raise RuntimeError("本段重试 %d 次仍失败：%s" % (attempts, last))
...
def collect_videos(paths, default_dir):
    ...
    for t in targets:
        ...
        if p.is_dir():
            ...
            log("这个文件格式不支持：%s" % p.name)
            log("  支持的格式：%s" % "、".join(sorted(MEDIA_EXT)))
    uniq = []
    for f in found:
        if f not in uniq:
            uniq.append(f)
...
def transcribe_video(video, args):
    ...
    all_items = []
    tag = hashlib.md5(str(video.resolve()).encode("utf-8")).hexdigest()[:12]
    chunk_file = Path(tempfile.gettempdir()) / ("bcut_chunk_%s.mp3" % tag)
    try:
        for i in range(total):
            ...
            if key in cache:
                ...
                utterances = transcribe_chunk(chunk_file, label="第 %d/%d 段" % (i + 1, total))
                cache[key] = utterances
                if not args.no_cache:
                    try:
                        cache_path.write_text(json.dumps(cache, ensure_ascii=False), encoding="utf-8")
                    except OSError as exc:
                        log("进度缓存写入失败（不影响本次处理）：%s" % exc)
...
def main():
    ...
    parser.add_argument("--interval", type=float, default=3.0, help="每段之间的间隔秒数，默认 3")
    ...
    if not videos:
        print()
        print("【没有找到可以处理的文件】")
        print("  支持的视频格式：%s" % "、".join(sorted(VIDEO_EXT)))
        print("  支持的音频格式：%s" % "、".join(sorted(AUDIO_EXT)))
        print()
        print("用法：把视频/音频文件、或者放着它们的文件夹，拖到本文件上；")
        print("      也可以直接双击本文件，它会处理自己所在的文件夹。")
        return 1
    print("待处理 : %d 个文件" % len(videos))
    ...
    for idx, video in enumerate(videos, 1):
        ...
        try:
            ...
            log("处理失败：%s: %s" % (type(exc).__name__, exc))
            code = 1
        if code == 0:
            ok += 1
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("如果看不懂这个错误，把上面几行发给我。")
        code = 1
        need_pause = True
    if code != 0:
        need_pause = True
    # 正常跑完不暂停；出错或没找到文件时停住，别让窗口一闪就没了
    if need_pause and sys.stdout.isatty():
        try:
            input("按回车退出……")
        except EOFError:
            pass
