...
#
# v1.1 起会检查接口有没有「假成功」：
#   腾讯这个免费接口偶尔会把原文原样吐回来，ret_code 还是 succ，
#   看着像翻好了、其实一个字没动。现在遇到这种会逐条重试；
#   重试后还是原样，就不写进缓存（免得脏数据固化，以后一直翻不出来）。
#
# 用法示例：
...
#   python 批量翻译字幕.py --no-verify             跳过「假成功」检查（跑得快一点）
...
ECHO_RETRIES = 2            # 疑似「原样返回」时，单独重试几次

_opener = None
_opener_lock = threading.Lock()
_print_lock = threading.Lock()
_echo_lock = threading.Lock()
ECHO_STATS = {"echoed": 0, "fixed": 0, "failed": 0}
...
def looks_echoed(src_text, out_text):
    """译文是不是把原文原样吐回来了（接口静默降级）"""
    a = re.sub(r"\s+", " ", (src_text or "").strip())
    b = re.sub(r"\s+", " ", (out_text or "").strip())
    if not a or a != b:
        return False
    if CJK_RE.search(a):
        return False        # 原文本来就是中文，翻成中文不变是正常的
    if not LATIN_RE.search(a):
        return False        # 纯符号、数字、语气词
    return True


def is_trivial(text):
    """根本没法翻的（空行、单个字符、纯符号数字），原样保留，不浪费请求。
    注意别把 so / oh / no 这种两字母的词也跳过了，它们是有译文的。"""
    s = (text or "").strip()
    if not s:
        return True
    if CJK_RE.search(s):
...
def retry_one(engine, text, src, dst, proxy):
    """单独重试一条（条数少的时候接口老实得多）"""
    if engine == "transmart":
        out = transmart_with_retry([text], src, dst, proxy)
        return out[0] if out else ""
    if engine == "youdao":
        try:
            return youdao_translate(text, src, dst, proxy)
        except Exception:  # noqa: BLE001
            return translate_with_retry(text, "auto", "zh-CN", proxy)
    return translate_with_retry(text, src, dst, proxy)
...
def translate_batch(batch, args, cache, cache_lock):
    ...
    for i, t in batch:
        if is_trivial(t):
            result[i] = t            # 太短，翻不出东西，原样留着
            continue
        with cache_lock:
            hit = cache.get(prefix + t)
            if hit and looks_echoed(t, hit):
                del cache[prefix + t]     # 之前存进去的脏数据，清掉重来
                hit = None
        if hit:
            result[i] = hit
        else:
    ...
    try:
        ...
        log("这一批没请求成功（%s），改逐条重试" % e)
        outs = []

    if len(outs) != len(texts):
        # 整批挂了：逐条救，救不回来就保留原文（不写缓存，下次重跑还会再试）
        rescued = []
        for t in texts:
            got = ""
            for _ in range(2):
                time.sleep(0.6)
                try:
                    got = (retry_one(engine, t, src, dst, proxy) or "").strip()
                except Exception as e:  # noqa: BLE001
                    log("单条失败：%s" % e)
                    continue
                if got:
            ...
            if not got:
                with _echo_lock:
                    ECHO_STATS["failed"] += 1
            rescued.append(got or t)
        outs = rescued

    final = [(o or "").strip() for o in outs]

    # v1.1：揪出「原样返回」的假成功，逐条重试
    if not args.no_verify:
        for k, (i, t) in enumerate(todo):
            if not looks_echoed(t, final[k]):
                continue
            fixed = final[k]
            for _ in range(ECHO_RETRIES):
                time.sleep(0.4)
                try:
                    cand = (retry_one(engine, t, src, dst, proxy) or "").strip()
                except Exception as e:  # noqa: BLE001
                    log("单条重试失败：%s" % e)
                    continue
                if cand and not looks_echoed(t, cand):
                    fixed = cand
                    break
            with _echo_lock:
                if fixed != final[k]:
    ...
    for (i, _), o in zip(todo, final):
        result[i] = o
    # 原样返回的不写缓存：下次重跑还能再试，不会被脏数据卡死
    with cache_lock:
        for (i, t), o in zip(todo, final):
            if looks_echoed(t, o) or o == t:
                continue      # 原样返回 / 兜底保留原文的，都不写缓存
            cache[prefix + t] = o
    return result
...
def process_srt(path: Path, args, cache, cache_lock, cache_path: Path):
    ...
    def work(b):
        ...
        try:
            ...
            log("这批出了意外（%s），先跳过，重跑会再试" % e)
            with _echo_lock:
                ECHO_STATS["failed"] += len(batch)
            return start, dict(batch)      # 兜底：先把原文填进去，别让成品缺行
    ...
    if not args.no_verify:
        with _echo_lock:
            echoed, fixed, failed = (ECHO_STATS["echoed"], ECHO_STATS["fixed"],
                                     ECHO_STATS["failed"])
        if fixed:
            log("接口原样返回、重试后翻出来的：%d 条" % fixed)
        if echoed:
            log("仍然没翻的：%d 条（多为乱码/专有名词，没写进缓存，下次重跑还会再试）" % echoed)
        if failed:
            log("请求没成功的：%d 条（保留了原文，没写进缓存，重跑会再试）" % failed)
...
def main():
    ...
    ap.add_argument("--no-verify", action="store_true",
                    help="跳过「接口原样返回」的检查与重试（跑得快一点，但可能漏翻）")
