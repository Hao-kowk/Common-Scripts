...
$PRESETS = [ordered]@{
    ...
    '4' = [pscustomobject]@{ Name = '很强'; Filter = 'acompressor=threshold=-22dB:ratio=14:attack=1:release=300' }
    '5' = [pscustomobject]@{ Name = '最强'; Filter = 'acompressor=threshold=-26dB:ratio=20:attack=1:release=300' }
}
    ...
    foreach ($it in $Items) {
        ...
        if ($item.PSIsContainer) {
            Get-ChildItem -LiteralPath $item.FullName -Recurse -File | ForEach-Object {
                # 以 _ 开头的子文件夹（_处理后、_试听 等）整体跳过
                $rel = $_.FullName.Substring($item.FullName.Length).TrimStart('\')
                $dirs = @($rel -split '\\' | Select-Object -SkipLast 1)
                $skip = $false
                foreach ($d in $dirs) { if ($d.StartsWith('_')) { $skip = $true; break } }
                if ((-not $skip) -and ($VIDEO_EXT -contains $_.Extension.ToLower()) -and ($_.BaseName -notmatch '_音量压缩$')) {
...
Write-Info '=========================================='
Write-Info (' 视频音量处理工具  v{0}' -f $VERSION)
Write-Info '=========================================='
...
if ($Mode -eq 0) {
    Write-Info ''
    Write-Info '选择压缩强度（数字越大，大声的部分压得越低）：'
    Write-Info '  1 = 温和    只压最响的，改动最小'
    Write-Info '  2 = 明显'
    Write-Info '  3 = 强力    原来的默认档，大声处约压到 -16 dB'
    Write-Info '  4 = 很强    大声处约压到 -20 dB'
    Write-Info '  5 = 最强    推荐，直接回车就是它，大声处约压到 -25 dB'
    Write-Info '  9 = 只检查，不处理'
    $ans = ''
    for ($i = 0; $i -lt 5; $i++) {
        Write-Host '请输入 1 / 2 / 3 / 4 / 5 / 9，直接回车 = 5：' -NoNewline
        $a = Read-Host
        if ($null -eq $a) { $a = '' }
        $a = $a.Trim().TrimStart([char]0xFEFF).Trim()
        if ($a -eq '') { $ans = '5'; break }
        if ($a -match '^[123459]') { $ans = $a.Substring(0, 1); break }
        Write-Warn2 '输入无效，请输入 1、2、3、4、5、9，或直接回车。'
    }
    if ($ans -eq '') { Write-Bad '多次输入无效，已退出。'; exit 1 }
    $Mode = [int]$ans
}

if ((1, 2, 3, 4, 5, 9) -notcontains $Mode) {
...
foreach ($f in $files) {
    ...
    $of1 = Join-Path $od ($bn + '_音量压缩.mp4')
    $of2 = Join-Path $od ($bn + '.mp4')
    if ((Test-Path -LiteralPath $of1) -or (Test-Path -LiteralPath $of2)) { $existing++ } else { $pending.Add($f) }
}
...
foreach ($file in $files) {
    ...
    $line = '{0}  {1}  [{2}]  源 平均 {3}/峰值 {4} → 输出 平均 {5}/峰值 {6}  时长 {7}  结论：{8}  (v{9})' -f `
        ...
        (Format-Sec ([int]$srcDur)), $verdict, $VERSION
